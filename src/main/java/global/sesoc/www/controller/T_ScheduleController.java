...
public class T_ScheduleController {
   ...
   public String Calendar(Model model , HttpSession session) {
      ...
      List<T_Schedule> list=T_ScheduleRepository.selectUserAllSchedule(userId);
      List<T_Friend> fList=T_FriendRepository.myFriendList(userId);
      List<String> friList=new ArrayList<String>();
      for (int i = 0; i < fList.size(); i++) {
         if(!fList.get(i).getFriAccepter().equals(userId)) {
            friList.add(fList.get(i).getFriAccepter());
         }
         if(!fList.get(i).getFriRequester().equals(userId)) {
            friList.add(fList.get(i).getFriRequester());
         }
      }
      ...
      int x = 0;
        while (x < 1000) {    
            ...
            for (int i = 0; i < friList.size(); i++) {
                if (!resultList.contains(friList.get(i))) {
                    resultList.add(friList.get(i));
                }
            }
            x++;
        }
   ...
   @ResponseBody
   @RequestMapping(value="/insertShareShedule",method=RequestMethod.POST)
   public int insertShareShedule(@RequestBody T_Request request , HttpSession session) {
      String userId=(String)session.getAttribute("loginId");
      T_Planner planner=new T_Planner(); 
      System.out.println("11::"+request);
      String title=userId+"님과"+request.getRequester()+"님의 플래너";
      planner.setPlaTitle(title);
      System.out.println("planner??"+planner);
      T_PlannerRepository.insertPlanner(planner);
      int plaNum=T_PlannerRepository.maxPlanum();
      System.out.println("222"+plaNum);
      T_Plist userPlist= new T_Plist();   
      userPlist.setUserId(userId);    userPlist.setPlaNum(plaNum);
      T_Plist friPlist= new T_Plist();   
      friPlist.setUserId(request.getRequester()); friPlist.setPlaNum(plaNum);
      System.out.println("e33=="+userPlist+"11111::"+friPlist);
      T_PlistRepository.insertPlist(userPlist);
      T_PlistRepository.insertPlist(friPlist);
      
      int result=0;
      List<T_Schedule> shareSchedule=T_ScheduleRepository.selectMixSchedule(userId, request.getRequester());
      System.out.println("4444::"+shareSchedule);
      for (int i = 0; i < shareSchedule.size(); i++) {
         shareSchedule.get(i).setPlaNum(plaNum);
         System.out.println("55::::"+shareSchedule.get(i));
         result=T_ScheduleRepository.insertShareSchedule(shareSchedule.get(i));
      }
